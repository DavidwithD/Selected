...
/**
 * Store one selection.
 * source is 'selection' or 'clipboard'. A clipboard record has no source page,
 * so it gets the pseudo host 'clipboard' and the site filter can select it.
...
export async function addSelection({
  ...
  const record = {
    ...
    url,
    title,
    host: source === 'clipboard' ? 'clipboard' : hostOf(url),
    ts,
    source,
  };
  ...
  // A clipboard record has an empty url. Without the url check, one clipboard
  // record could replace another.
...
/** Text of the newest record, or '' when the store is empty. */
export async function newestText() {
  const db = await openDb();
  const store = db.transaction(STORE, 'readonly').objectStore(STORE);
  const cursor = await request(store.index('ts').openCursor(null, 'prev'));
  return cursor ? cursor.value.text : '';
}
